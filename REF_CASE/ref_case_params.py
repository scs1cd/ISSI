...
T_cmb                      = 1900  #Initial temperature of the CMB. Can specify Tcen (temperature at the center of the core) instead. Float.
...
core_solid_density_params  = [7211]  #Inner core density polynomials (radial). List(float)
core_liquid_density_params = [7211]  #Outer core density polynomials (radial). List(float)
...
core_cp_params             = [780]  #Core specific heat capacity pressure polynomial coefficients. List(Float)
core_conductivity_params   = [40]  #List, first element is a string followed by the core thermal conductivity polynomial coefficients. The string (either 'r'/'T'/'P') indicates if the polynomials are radial, temperature, or pressire coefficients. List(string, Float, Float...)
core_melting_params        = ['WN', 1990.48621274289, 1, -0.00218881211768781/1e9, 3.77133942042793e-07/1e18]  #List, first element is a string followed by the constants used for the melting temperature parameterisation. The string element indicates the parameterisation to be used, if no string is given, 'AL' (method of Alfe et al. 2002) is assumed. See melting_curve() in ./routines/chemistry.py for possible options.
...
#core: development optional parameters
...
core_adiabat_params = [1, -1e-5/2000] #(Default: None) None or List(float). Radial polynomial coefficients for the core adiabat normalised to T(r=0). If left as None, these will be calculated by fitting to a numerically integrated adiabatic gradient using given alpha/cp/g profiles.
