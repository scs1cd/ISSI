...
q_cmb_array = [1]#[1, 5, 10]        #mW/m^2
S_array     = [0.25]#[0.25, 0.3, 0.35]

#Different timesteps for different heat flows to speed up calculation (yrs).
timesteps   = [5e5]#[5e5, 1e5, 1e4]  
...
for S in S_array:
   ...
   for q_cmb, _dt in zip(q_cmb_array, timesteps):
      ...
      dt = _dt*prm.ys
      while not model.critical_failure and model.it < 10:
      ...
      print(model.critical_failure_reason)
