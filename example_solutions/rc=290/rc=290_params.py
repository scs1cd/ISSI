...
T_cmb                      = 2050  #Initial temperature of the CMB. Can specify Tcen (temperature at the center of the core) instead. Float.
...
core_solid_density_params  = [7200]  #Inner core density polynomials (radial). List(float)
core_liquid_density_params = [7200]  #Outer core density polynomials (radial). List(float)
ri                         = 0  #Initial inner core radius. Float
r_cmb                      = 290e3  #CMB radius. Float
...
core_adiabat_params = [1, -1e-3/T_cmb] #(Default: None) None or List(float). Radial polynomial coefficients for the core adiabat normalised to T(r=0). If left as None, these will be calculated by fitting to a numerically integrated adiabatic gradient using given alpha/cp/g profiles.
...
mf_l = [0.02] #(Default None) Starting mole fraction of light element in the core. If specified it will overrule conc_l.
