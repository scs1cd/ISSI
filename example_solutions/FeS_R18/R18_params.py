...
T_cmb                      = 1800  # Initial temperature of the CMB. Can specify Tcen (temperature at the center of the core) instead. 
                                   # Float.

conc_l                     = [0.316]  # Initial light element mass fraction of the core. Preserve order consistency with all other light 
                                    # element property lists. List(float).

#Assumes linear variation between values quoted between pure iron and top end of S range. 
core_solid_density_params  = [4831]  # Inner core density polynomials (radial). List(float)

core_liquid_density_params = [4831]  # Outer core density polynomials (radial). List(float)

ri                         = 0  # Initial inner core radius. Float

r_cmb                      = 999e3  # CMB radius. Float

core_alpha_T_params        = [9e-5]  # Core thermal expansivity pressure polynomial coefficients. List(Float)

core_cp_params             = [800] # Core specific heat capacity pressure polynomial coefficients. List(Float)

core_conductivity_params   = [40]  # List, first element is a string followed by the core thermal conductivity polynomial coefficients. 
...
core_melting_params        = ['RU', 1265, 11.15e-9, 3e-9, 1500.5, 29.6e-9, 0.247e-18]
...
entropy_melting_params     = [136 / (Na *1000/56)]
